...
import com.Jason.aclservice.entity.RolePermission;
import com.Jason.aclservice.entity.User;
import com.Jason.aclservice.helper.MemuHelper;
import com.Jason.aclservice.helper.PermissionHelper;
import com.Jason.aclservice.mapper.PermissionMapper;
import com.Jason.aclservice.service.PermissionService;
import com.Jason.aclservice.service.RolePermissionService;
...
//        });
    ...
    //获取全部菜单
    @Override
    public List<Permission> queryAllMenuCili() {
        //1 查询菜单表所有数据
        QueryWrapper<Permission> wrapper = new QueryWrapper<>();
        wrapper.orderByDesc("id");
        List<Permission> permissionList = baseMapper.selectList(wrapper);
        //2 把查询所有菜单list集合按照要求进行封装,即把列表数据封装成多级菜单
    ...
    //============递归删除菜单==================================
    @Override
    public void removeChildByIdCili(String id) {
...
//        baseMapper.deleteBatchIds(idList);
        ...
        while(!toDelete.isEmpty()){
            String tmp = toDelete.peek();
            int flag = 0;
            for(Permission p:allPermissions){
                if(p.getPid().equals(tmp)){
                    toDelete.push(p.getId());
                    flag =1;
                }
            }
            if(flag==0){
                baseMapper.deleteById(tmp);
            }
        }
