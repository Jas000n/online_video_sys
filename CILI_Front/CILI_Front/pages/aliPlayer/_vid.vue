<template>
  <div >
    ...
    <!--    影视评论-->
    <div class="mt50 commentHtml" style="overflow: scroll; height: 400px">
      <div>
...
export default {
  ...
  methods: {
    initPlayer() {
      vodApi.getVideoSourceID(this.vid).then((response) => {
        console.log(response);
        this.videoSourceID = response.data.data.videoSourceID;
        vodApi.getPlayURL(this.videoSourceID).then((result) => {
    ...
    initComment() {
      commentApi
        .getPageList(this.vid, this.page, this.limit)
        .then((response) => {
          console.log(response);
